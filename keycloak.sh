...
if [ $KEYCLOAK_IMPORT ]; then
    IMPORT_CONFIG=" -Dkeycloak.migration.action=import -Dkeycloak.migration.strategy=OVERWRITE_EXISTING -Dkeycloak.migration.provider=singleFile -Dkeycloak.migration.file=${BUILD_DIR}/${KEYCLOAK_IMPORT}"
fi
